                ...
                <table id="datatable" class="table table-bordered dt-responsive">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>{{$channel->name}}</td>
                            <td>
                                <a type="button" href="{{route('d.edit',$channel->id)}}" class="btn btn-sm btn-primary"> <i class="bx bx-pen"></i></a>
                                <a type="button" href="{{route('d.delete',$channel->id)}}" class="btn btn-sm btn-danger deletebtn"><i class="bx bx-x"></i></a>
                            </td>
                        </tr>
                    </tbody>
                    @endforeach
    ...
    <div class="col-4">
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{ isset($editData) ? route('dt', $editData->id) : route('d.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    @if(isset($editData))
                        @method('PUT')
                    @endif
